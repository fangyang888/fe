...
# female_students = [student for student in students if student["sex"] == "女"]

# 3. 创建一个新列表 `young_females`，包含所有年龄小于 25 岁的女生

# young_females = [student for student in students if student["age"] < 25 and student["sex"] == "女"]

# 4. 创建一个新列表 `chen_students`，包含所有姓"陈"的学生（提示：`name[0]` 可以获取姓名的第一个字）

# chen_students = [student for student in students if student["name"][0] == "陈"]

# 5. 创建一个新列表 `tel_end_with_1`，包含所有电话号码最后一位是 "1" 的学生（提示：字符串可以用索引访问最后一个字符，如 `tel[-1]`）

# tel_end_with_1 = [student for student in students if student["tel"][-1] == "1"]
...
# female_names = [student["name"] for student in students if student["sex"] == "女"]

# 8. 创建一个新列表 `female_contacts`，其中每个元素是一个字典，只包含 `name` 和 `tel` 两个键，仅包含女生的信息

# female_contacts = [{"name": student["name"], "tel": student["tel"]} for student in students if student["sex"] == "女"]
...
# print(f"所有学生的平均年龄: {average_age:.2f}")
...
student_796997 = next((student for student in students if student["id"] == 796997), None)
if student_796997:
    print(f"姓名: {student_796997['name']}, 电话号码: {student_796997['tel']}")
else:
    print("未找到")
# 为什么要用next 
# `next()` 函数在这里的作用是从一个迭代器中获取下一个元素。在这个例子中，我们使用了一个生成器表达式 `(student for student in students if student["id"] == 796997)` 来遍历 `students` 列表，并查找 `id` 为 `796997` 的学生。
# - 如果找到了符合条件的学生，`next()` 会返回该学生的字典
# - 如果没有找到符合条件的学生，`next()` 会返回 `None`，因为我们提供了一个默认值 `None` 作为第二个参数。

# 把next 去掉可以吗 
# 可以去掉 `next()`，但是需要使用其他方法来查找学生。例如，可以使用一个简单的循环来遍历 `students` 列表，找到符合条件的学生。下面是一个不使用 `next()` 的示例：

# ```python
# student_796997 = None
# for student in students:
#     if student["id"] == 796997:
#         student_796997 = student
#         break
...
# is_year_than_28_male = next((student for student in students if student['age'] > 28 and student['sex'] == "男"), None)
# if is_year_than_28_male:
#     print(True)
#     print(is_year_than_28_male['name'])
# else:
#     print(False)
# next 只能找到一个  那样子有问题

# 用any 帮我实现一下
# is_year_than_28_male_any = any(student['age'] > 28 and student['sex'] == "男" for student in students)
# if is_year_than_28_male_any:
#     print(True)
#     # 打印所有年龄大于28岁的男生姓名
#     for student in students:
#         if student['age'] > 28 and student['sex'] == "男":
#             print(student['name'])
# else:
#     print(False)
...
is_all_females_under_28 = all(student['age'] <= 28 for student in students if student['sex'] == "女")
if is_all_females_under_28:
    print(True)
else:
    print(False)
