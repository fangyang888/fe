...
// We want to predict exactly 10 numbers to kill.
for (let wHot = 5; wHot <= 40; wHot += 5) {
  for (let topHot = 0; topHot <= 10; topHot += 2) {
    for (let wCold = 5; wCold <= 40; wCold += 5) {
      ...
      const acc = (totalCorrect / totalPredicted) * 100;
      if (acc > bestAcc) {
          bestAcc = acc;
          bestParams = { wHot, topHot, wCold, topCold };
      }
    }
  }
}
console.log('Best Acc:', bestAcc, bestParams);
