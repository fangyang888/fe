...
async function main() {
  ...
  const results = {
    exclude1: { correct: 0, total: 300 },
    exclude1_cb: { correct: 0, total: 0, skipped: 0 },
    exclude2: { correct: 0, total: 300 },
    exclude2_cb: { correct: 0, total: 0, skipped: 0 },
    exclude3: { correct: 0, total: 300 }
  };
  ...
  for (let i = N - testPeriods; i < N; i++) {
    ...
    // Evaluate Exclude-1 (Only take the #1 recommendation)
    const p1 = selectedPredictions[0].num;
    if (!currentDraw.includes(p1)) {
      results.exclude1.correct++;
    }

    // Evaluate Exclude-1 with Circuit Breaker (Require #1 vote count >= 3)
    if (selectedPredictions[0].votes >= 3) {
      results.exclude1_cb.total++;
      if (!currentDraw.includes(p1)) {
        results.exclude1_cb.correct++;
      }
    } else {
      results.exclude1_cb.skipped++;
    }

    // Evaluate Exclude-2 (Take top 2 recommendations)
    const p2_1 = selectedPredictions[0].num;
    const p2_2 = selectedPredictions[1].num;
    if (!currentDraw.includes(p2_1) && !currentDraw.includes(p2_2)) {
      results.exclude2.correct++;
    }

    // Evaluate Exclude-2 with Circuit Breaker (Require sum of votes >= 5)
    const votesSum = selectedPredictions[0].votes + selectedPredictions[1].votes;
    if (votesSum >= 5) {
      results.exclude2_cb.total++;
      if (!currentDraw.includes(p2_1) && !currentDraw.includes(p2_2)) {
        results.exclude2_cb.correct++;
      }
    } else {
      results.exclude2_cb.skipped++;
    }

    // Evaluate Exclude-3 (Take top 3 recommendations)
    const p3_1 = selectedPredictions[0].num;
    const p3_2 = selectedPredictions[1].num;
    const p3_3 = selectedPredictions[2].num;
    if (!currentDraw.includes(p3_1) && !currentDraw.includes(p3_2) && !currentDraw.includes(p3_3)) {
      results.exclude3.correct++;
    }
  }
  ...
  console.log('========================================================================');
  console.log('排除模式与维度                         | 预测期数/总期数  | 空仓率   | 排除全对概率');
  console.log('------------------------------------------------------------------------');
  
  const printRow = (label, obj) => {
    const rate = ((obj.correct / obj.total) * 100).toFixed(2);
    const skipRate = obj.skipped !== undefined ? ((obj.skipped / 300) * 100).toFixed(1) + '%' : '0.0%';
    const countStr = `${obj.correct}/${obj.total}`;
    console.log(`${label.padEnd(38)} | ${countStr.padStart(15)} | ${skipRate.padStart(6)} | ${rate.padStart(11)}%`);
  };

  printRow('排除三码 (Exclude-3, Baseline)', results.exclude3);
  printRow('排除二码 (Exclude-2, Baseline)', results.exclude2);
  printRow('排除二码 + 熔断 (共识票数和 >= 5)', results.exclude2_cb);
  printRow('排除一码 (Exclude-1, Baseline)', results.exclude1);
  printRow('排除一码 + 熔断 (第一杀号得票 >= 3)', results.exclude1_cb);
  console.log('========================================================================\n');
