...
async function runBacktestForWindow(windowSize) {
  ...
  for (let i = N - testPeriods; i < N; i++) {
    ...
    for (let k = 0; k < 3; k++) {
      if (rankedNumbers[k]) {
        ...
        selectedPredictions.push(item.num);
    ...
    if (selectedPredictions.length < 3) {
      for (const ruleStat of sortedStats) {
        ...
        if (num && !usedPreds.has(num)) {
          usedPreds.add(num);
          selectedPredictions.push(num);
        }
      }
    }

    // 验证排除一码是否成功
    if (!currentDraw.includes(selectedPredictions[0])) {
      correctCount1++;
    }

    // 验证排除三码是否成功
    const failed3 = selectedPredictions.slice(0, 3).filter(p => currentDraw.includes(p));
    if (failed3.length === 0) {
      correctCount3++;
    }
  }
  ...
  return {
    exclude1Rate: ((correctCount1 / testPeriods) * 100).toFixed(2),
    exclude3Rate: ((correctCount3 / testPeriods) * 100).toFixed(2)
  };
}

async function main() {
  const windows = [2, 3, 5, 10, 20, 30, 50, 100];
  console.log('\n========================================================================');
  console.log(`回测样本数：300 期 | 对比不同历史滑动窗口大小时的排除成功率`);
  console.log('========================================================================');
  console.log('滑动窗口大小 (最近几期数据) | 排除一码全对概率 (Exclude-1) | 排除三码全对概率 (Exclude-3)');
  console.log('------------------------------------------------------------------------');
  
  for (const w of windows) {
    const res = await runBacktestForWindow(w);
    console.log(
      `最近 ${String(w).padStart(3)} 期规律           | ${res.exclude1Rate.padStart(20)}% | ${res.exclude3Rate.padStart(20)}%`
    );
  }
  console.log('========================================================================\n');
}
