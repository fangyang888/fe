...
@Injectable()
export class PredictorOptService implements OnModuleDestroy {
  constructor(
  ...
  private memoFeatureRows = new BoundedCache<string, any>(1000);
  private memoTransitionScores = new BoundedCache<string, any>(1000);
  private memoHotPickKill5Candidates = new BoundedCache<string, any>(500);
  private memoRecentModelKillRates = new BoundedCache<string, any>(1000);
  private memoProbabilityKillPredictions = new BoundedCache<string, any>(1000);
  private memoLowRiskKillPredictions = new BoundedCache<string, any>(1000);
  private memoHotPickResponse = new BoundedCache<string, any>(100);
  private memoAdaptiveThreshold = new BoundedCache<string, number>(1000);
  private memoOptimizedPredictions = new BoundedCache<string, any>(1000);
  private memoRollingContributions = new BoundedCache<string, any>(1000);
  ...
  private lastBestEngineMode = 'ensemble';
  private lastBestEnginePerformance: any = null;
  private lastBestAllCorrectModelPerformance: any = null;
  private lastBestHybridVariant: any = null;
  private lastBestAppearWeights: any = null;
  ...
  private checkAndClearCache(currentHistLength: number, source: HistorySourceType = 'default') {
    ...
    if (source !== this.lastHistorySource || currentHistLength < this.lastHistLength) {
      ...
      this.memoFeatureRows.clear();
      this.memoTransitionScores.clear();
      this.memoHotPickKill5Candidates.clear();
      this.memoRecentModelKillRates.clear();
      this.memoProbabilityKillPredictions.clear();
      this.memoLowRiskKillPredictions.clear();
      this.memoHotPickResponse.clear();
      this.memoAdaptiveThreshold.clear();
      this.memoOptimizedPredictions.clear();
      this.memoRollingContributions.clear();

      this.lastBestEngineMode = 'ensemble';
      this.lastBestEnginePerformance = null;
      this.lastBestAllCorrectModelPerformance = null;
      this.lastBestHybridVariant = null;
      this.lastBestAppearWeights = null;
    }
  ...
  async getHotPickPredictionResponse(type?: string, options: { forceRefresh?: boolean } = {}) {
    ...
    if (!options.forceRefresh && this.memoHotPickResponse.has(responseCacheKey)) {
      const cachedMemo = this.memoHotPickResponse.get(responseCacheKey);
      return {
        ...cachedMemo,
        cacheMeta: {
    ...
    const cached = options.forceRefresh ? null : await this.getJsonCache<any>(responseCacheKey);
    if (cached) {
      this.memoHotPickResponse.set(responseCacheKey, cached);
      return {
        ...cached,
        cacheMeta: {
          ...(cached.cacheMeta || {}),
    ...
    this.memoHotPickResponse.set(responseCacheKey, finalResponse);
  ...
  private getRecentModelKillRates(hist: number[][], evalPeriods = 30, useWeights?: any) {
    const cacheKey = `${this.getHistArrayCacheKey(hist)}:${evalPeriods}:${useWeights ? 'custom' : 'auto'}`;
    if (this.memoRecentModelKillRates.has(cacheKey)) {
      return this.memoRecentModelKillRates.get(cacheKey);
    }
    ...
    const weights = useWeights || this.getTrainedAppearWeights(hist).weights;
    ...
    for (let i = start; i < hist.length; i++) {
      ...
      for (const prediction of this.getProbabilityKillPredictions(subHist, 10, weights)) {
    ...
    this.memoRecentModelKillRates.set(cacheKey, stats);
    return stats;
  }
  ...
  private getHotPickKill5Candidates(hist: number[][], occurrenceStats: any, threshold = 94.0, fastMode = false): any {
    ...
    const probabilityPreds = this.getProbabilityKillPredictions(hist, 49, fastMode ? this.lastBestAppearWeights : undefined);
    ...
    const featureByNum = new Map(featureRows.map((row) => [row.n, row.features]));
    ...
    this.memoHotPickKill5Candidates.set(cacheKey, { candidates });
    return res;
  }

  private getAdaptiveHotPickKill5Threshold(hist: number[][]): number {
    if (hist.length < 45) {
      return 94.0;
    }

    const cacheKey = this.getHistArrayCacheKey(hist);
    if (this.memoAdaptiveThreshold.has(cacheKey)) {
      return this.memoAdaptiveThreshold.get(cacheKey)!;
    }
    ...
    for (let i = start; i < hist.length; i++) {
      ...
      const { candidates } = this.getHotPickKill5Candidates(subHist, occurrenceStats, 94.0, true);
      const actualSet = new Set(hist[i]);
    ...
    this.memoAdaptiveThreshold.set(cacheKey, bestThreshold);
  ...
  private getTrainedAppearWeights(hist: number[][]) {
    const key = hist.length;
    if (this.memoAppearWeights.has(key)) {
      const cached = this.memoAppearWeights.get(key);
      this.lastBestAppearWeights = cached.weights;
      return cached;
    }
    const res = this.trainAppearWeights(hist);
    this.memoAppearWeights.set(key, res);
    this.lastBestAppearWeights = res.weights;
    return res;
  }
  ...
  private getProbabilityKillPredictions(hist: number[][], count = 10, useWeights?: any) {
    const cacheKey = `${this.getHistArrayCacheKey(hist)}:${count}:${useWeights ? 'custom' : 'auto'}`;
    if (this.memoProbabilityKillPredictions.has(cacheKey)) {
      return this.memoProbabilityKillPredictions.get(cacheKey);
    }
    const weights = useWeights || this.getTrainedAppearWeights(hist).weights;
    const res = this.getProbabilityKillPredictionsWithWeights(
      hist,
      weights,
      count,
    );
    this.memoProbabilityKillPredictions.set(cacheKey, res);
    return res;
  }
  ...
  private getLowRiskKillPredictions(hist: number[][], count = 10) {
    const cacheKey = `${this.getHistArrayCacheKey(hist)}:${count}`;
    if (this.memoLowRiskKillPredictions.has(cacheKey)) {
      return this.memoLowRiskKillPredictions.get(cacheKey);
    }
    const res = this.getLowRiskKillScores(hist)
      .slice(0, count)
      .map((s, i) => ({
        n: s.n,
        tier: i < 3 ? 'S1' : i < 6 ? 'S2' : 'S3',
    ...
    this.memoLowRiskKillPredictions.set(cacheKey, res);
    return res;
  }
  ...
  private getHistoricalLearningScores(hist: number[][]): AppearScore[] {
    ...
    const currentBuckets = new Map<number, ReturnType<PredictorOptService['bucketLearningFeatures']>>();
  ...
  private getHotPickFeatureRows(hist: number[][]) {
    const cacheKey = this.getHistArrayCacheKey(hist);
    if (this.memoFeatureRows.has(cacheKey)) {
      return this.memoFeatureRows.get(cacheKey);
    }
    ...
    this.memoFeatureRows.set(cacheKey, rows);
    return rows;
  }

  private getHotPickTransitionScores(hist: number[][]) {
    const cacheKey = this.getHistArrayCacheKey(hist);
    if (this.memoTransitionScores.has(cacheKey)) {
      return this.memoTransitionScores.get(cacheKey);
    }
    ...
    for (let n = 1; n <= 49; n++) {
      let markovSum = 0;
      for (const a of last) {
        markovSum += counts[a] > 0 ? (matrix[a][n] + alpha) / (counts[a] + 7 * alpha) : this.randomAppearProb;
      }
      ...
      for (const a of prev) {
        for (const b of last) {
          ...
          pairSum += (7 * (row[n] + alpha)) / (total + 49 * alpha);
          pairCount++;
        }
      }
    ...
    const result = { markov, markov2 };
    this.memoTransitionScores.set(cacheKey, result);
    return result;
  }
  ...
  private diversifyHotPickCandidates(candidates: any[], count: number) {
    ...
    while (selected.length < count && pool.length > 0) {
      ...
      for (let i = 0; i < pool.length; i++) {
        ...
        const tailCount = tails.get(tail) || 0;
        const zoneCount = zones.get(zone) || 0;
        ...
        const penalizedScore = candidate.score * tailFactor * zoneFactor;
      ...
      const tail = chosen.n % 10;
      const zone = Math.floor((chosen.n - 1) / 10);
  ...
  private getHotPickRollingContributions(
    ...
    const histKey = this.getHistArrayCacheKey(hist);
    const cacheKey = `${histKey}:${count}:${strategy}:${diversified}:${poolSize}:${evalPeriods}`;
    if (this.memoRollingContributions.has(cacheKey)) {
      return this.memoRollingContributions.get(cacheKey);
    }
    ...
    this.memoRollingContributions.set(cacheKey, res);
    return res;
  }
  ...
  private getOptimizedHotPickPredictions(
    ...
    const histKey = this.getHistArrayCacheKey(hist);
    const cacheKey = `${histKey}:${count}:${strategy}:${diversified}`;
    if (this.memoOptimizedPredictions.has(cacheKey)) {
      return this.memoOptimizedPredictions.get(cacheKey);
    }
    ...
    this.memoOptimizedPredictions.set(cacheKey, res);
    return res;
  }
  ...
  private addVariantResult(
    tracker: ReturnType<PredictorOptService['createVariantTracker']>,
    result: ReturnType<PredictorOptService['scoreKillPrediction']>,
    actual: number[],
    periodOffset: number,
  ...
  private backtestEngineModels(
    ...
    const variants: Record<string, ReturnType<PredictorOptService['createVariantTracker']>> = {
  ...
  private runKillEngine(hist: number[][], killCount: number, fastMode = false): KillEngineResult {
    const cacheKey = `${this.getHistArrayCacheKey(hist)}:${killCount}:${fastMode ? 'fast' : 'full'}`;
    if (this.memoKillEngine.has(cacheKey)) {
      return this.memoKillEngine.get(cacheKey)!;
    }

    if (fastMode && this.lastBestEngineMode) {
      const models = this.getEngineModelOutputs(hist);
      const finalModelPerformance =
        this.lastBestEngineMode === 'ensemble-strict-hard'
          ? this.lastBestAllCorrectModelPerformance || []
          : this.lastBestEnginePerformance || [];
      const predictions =
        this.lastBestEngineMode === 'ensemble-strict-hard'
          ...
          : this.lastBestEngineMode === 'probability'
            ? this.getProbabilityKillPredictions(hist, killCount, this.lastBestAppearWeights)
            : this.lastBestEngineMode === 'low-risk'
      ...
      const result = {
        ...
        stats: null,
        debug: {
          mode: `${this.lastBestEngineMode}-fast`,
          reason: 'fastmode-reused',
        },
      };
    ...
    this.lastBestEngineMode = backtest.selectedMode;
    this.lastBestEnginePerformance = backtest.modelPerformance;
    this.lastBestAllCorrectModelPerformance = backtest.allCorrectModelPerformance;
  ...
  private buildAdaptiveHybridKill10(hist: number[][], modelPredictions: any[], fastMode = false) {
    const cacheKey = `${hist.length}:${hist[hist.length - 1]?.join(',') || ''}:${fastMode ? 'fast' : 'full'}`;
    if (this.memoHybridKill10.has(cacheKey)) {
      return this.memoHybridKill10.get(cacheKey);
    }

    if (fastMode && this.lastBestHybridVariant) {
      const best = this.lastBestHybridVariant;
