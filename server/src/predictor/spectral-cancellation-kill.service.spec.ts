import { SpectralCancellationKillService } from './spectral-cancellation-kill.service';

const buildHistory = (count: number) =>
  Array.from({ length: count }, (_, index) => {
    const year = index < 365 ? 2024 : index < 730 ? 2025 : 2026;
    const offset = year === 2024 ? index : year === 2025 ? index - 365 : index - 730;
    const start = (index * 7) % 49;
    const numbers = Array.from({ length: 7 }, (_, position) => ((start + position) % 49) + 1);
    return {
      id: index + 1,
      year,
      No: offset + 1,
      n1: numbers[0],
      n2: numbers[1],
      n3: numbers[2],
      n4: numbers[3],
      n5: numbers[4],
      n6: numbers[5],
      n7: numbers[6],
    };
  });

describe('SpectralCancellationKillService', () => {
  it('builds a strictly walk-forward report without claiming the target rate', async () => {
    const historyService = { findAll: jest.fn().mockResolvedValue(buildHistory(800)) };
    const service = new SpectralCancellationKillService(historyService as any);

    const result = (await service.getPrediction()) as any;

    expect(result.strategy).toMatchObject({
      key: 'multiResolutionSpectralCancellationV1',
      family: 'signal-processing',
      targetRate: 0.95,
    });
    expect(result.prediction.number).toBeGreaterThanOrEqual(1);
    expect(result.prediction.number).toBeLessThanOrEqual(49);
    expect(result.spectrum).toHaveLength(49);
    expect(result.backtests.backtest500.count).toBe(500);
    expect(result.validation.start).toEqual({ year: 2026, No: 199 });
    expect(['qualified-signal', 'research-watch']).toContain(result.status);
  });

  it('requires enough history for a complete 500-period backtest', async () => {
    const historyService = { findAll: jest.fn().mockResolvedValue(buildHistory(691)) };
    const service = new SpectralCancellationKillService(historyService as any);

    await expect(service.getPrediction()).resolves.toMatchObject({
      status: 'insufficient-history',
      historyCount: 691,
    });
  });
});
