...
for (let i = hist.length - testPeriods; i < hist.length; i++) {
  ...
  // Strategy: Count occurrences in recent 20 periods, predict top 10 hottest
  const counts = Array(50).fill(0);
  const recent = subHist.slice(-20);
...
console.log('Accuracy (Hottest Recent 20):', (totalCorrect / totalPredicted) * 100 + '%');
