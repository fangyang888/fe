...
const COLORS = ["#f4c15d", "#61d4a8", "#79a7ff", "#d497ff", "#ff8e78"];
const WINDOWS = [10, 20, 50, 100, 200];
const pct = (data) => data?.count ? `${(data.successRate * 100).toFixed(1)}%` : "--";
const specialPct = (data) => data?.count && typeof data.specialCodeMissRate === "number" ? `${(data.specialCodeMissRate * 100).toFixed(1)}%` : "--";
      ...
      {WINDOWS.map((window) => {
        ...
        return (
          <div key={window}>
            ...
            <small>7码未出现 {pct(data)}</small>
            <em>特别码未出现 {specialPct(data)} · {data.specialCodeMissCount ?? '--'}/{data.count}</em>
          </div>
        );
      })}
    </div>
    ...
    <section className="aas-algorithm" style={{ "--aas-accent": color }}>
      ...
      <div className="aas-audit-grid">
        <article>
          ...
          <em>特别码未出现 {specialPct(algorithm.observedValidation)}</em>
        </article>
        <article>
          <span>212期起真实前瞻</span>
          ...
          <em>特别码未出现 {specialPct(algorithm.forwardValidation)}</em>
        </article>
      ...
      <details className="aas-details">
        ...
        <div className="aas-frozen">
          ...
          {WINDOWS.map((window) => {
            ...
            return <div key={window}><span>{window}期</span><strong>{data.successCount}/{data.count}</strong><small>{pct(data)}</small><em>特别码 {specialPct(data)}</em></div>;
          })}
        </div>
  ...
  return (
    <main className="aas-page">
      ...
      <div className="aas-shell">
        ...
        {!error && !data && <div className="aas-message">正在逐期计算五个算法…</div>}
        ...
        {data?.algorithms && <div className="aas-overview">{data.algorithms.map((algorithm, index) => <article key={algorithm.key} style={{ "--aas-accent": COLORS[index] }}><span>{algorithm.name}<i>采用 {algorithm.prediction.selectedBase}</i></span><strong>{algorithm.prediction.display}</strong><small>近200期7码未出现 {algorithm.rollingBacktests.backtest200.successCount}/{algorithm.rollingBacktests.backtest200.count} · {pct(algorithm.rollingBacktests.backtest200)}</small><small>特别码未出现 {specialPct(algorithm.rollingBacktests.backtest200)}</small></article>)}</div>}
