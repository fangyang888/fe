...
const pct = (value, count) => count > 0 && typeof value === 'number'
  ? `${(value * 100).toFixed(1)}%`
  : '--';
const specialPct = (data) => data?.count && typeof data.specialCodeMissRate === 'number'
  ? `${(data.specialCodeMissRate * 100).toFixed(1)}%`
  : '--';
...
function Metric({ label, data, accent = false }) {
  return <div className={`ais-metric${accent ? ' accent' : ''}`}>
    ...
    <small>{data?.count ? `${data.successCount}/${data.count} 期未在7码中出现` : '暂无样本'}</small>
