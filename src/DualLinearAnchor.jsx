...
const specialRate = value => value?.count && typeof value.specialCodeMissCount === 'number' ? `${(100 * value.specialCodeMissCount / value.count).toFixed(1)}%` : '—';
function SpecialMetric({ value }) {
  return <em className="dla-special">特别码未出现 {specialRate(value)}{value?.count ? ` · ${value.specialCodeMissCount}/${value.count}` : ''}</em>;
}
function LiveStats({ live, algorithmKey }) {
  const metric = live?.algorithms?.find(a => a.key === algorithmKey);
  return <div className="dla-live"><h3>{live?.start ? `${period(live.start)}期起留档实战` : '留档实战 · 等待首期预测'}</h3>
    <p>{metric?.count ? `7码未出现 ${rate(metric)} · ${metric.successCount}/${metric.count}` : '暂无已结算样本，成功率 —'}</p>
    <SpecialMetric value={metric} />
    <small>{metric?.pendingCount ? `${metric.pendingCount}期已留档，待数据库收录开奖。` : ''}{live?.message || '切换最新连续数据后保存下一期预测。'}</small>
    {metric?.rows?.length ? <details><summary>查看{metric.rows.length}期留档</summary><div className="dla-table"><table><thead><tr><th>期号</th><th>留档时间</th><th>预测</th><th>7码结果</th><th>特别码结果</th></tr></thead><tbody>{metric.rows.map(row => <tr key={period(row.target)}><td>{period(row.target)}</td><td>{new Date(row.savedAt).toLocaleString('zh-CN')}</td><td>{ball(row.number)}</td><td>{row.success === null ? '待开奖' : row.success ? '成功' : '失败'}{row.revised ? '（开奖数据后续有修订）' : ''}</td><td>{row.specialCodeMiss === null ? '—' : row.specialCodeMiss ? '未出现' : '出现'}</td></tr>)}</tbody></table></div></details> : null}
  </div>;
}
...
export default function DualLinearAnchor() {
  ...
  return <main className="dla-page"><div className="dla-shell">
    <header className="dla-header"><div><span className="dla-eyebrow">ANCHORS & TWO-PERIOD SELECTION</span><h1>14 / 100 双锚点</h1><p>两个固定公式 + 两种前两期动态择优，每种策略各排除一个号码。</p></div>
      ...
      <button className="dla-refresh" onClick={() => setAttempt(n => n + 1)}>刷新统计</button>
    </header>
    <p className="dla-note">公式与动态规则均在截至243期的历史上研究、筛选。历史全对不代表未来全对；成功指该号码未出现在当期全部7个开奖号中。动态策略仅使用前两期信息，同分选100期公式。</p>
    <p className="dla-meta">特别码未出现率：单杀号码与第7位不同的已结算比例。实战只统计先留档、后入库的期次，最新已开奖期不补算。</p>
    ...
    {data?.algorithms?.map((algorithm, index) => <section className={`dla-card dla-card-${index}`} key={algorithm.key}>
      ...
      <div className="dla-windows">{algorithm.windows.map(window => <article key={window.window}><span>近{window.window}期</span><strong>{rate(window)}</strong><small>{window.successCount} / {window.count} 期成功</small><SpecialMetric value={window} /></article>)}</div>
      <LiveStats live={data.live} algorithmKey={algorithm.key} />
      <div className="dla-research"><h3>截至243期的历史研究</h3>{algorithm.researchWindows.map(w => <p key={w.window}>近{w.window}期 {w.successCount}/{w.count}<SpecialMetric value={w} /></p>)}<small>244期起历史复算：{algorithm.afterResearch.count ? `${algorithm.afterResearch.successCount}/${algorithm.afterResearch.count} · ${rate(algorithm.afterResearch)}` : '此统计范围内暂无记录'}。此处不作为提前保存的前瞻预测。</small><SpecialMetric value={algorithm.afterResearch} /></div>
