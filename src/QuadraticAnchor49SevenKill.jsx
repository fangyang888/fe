...
function Metric({ label, data, accent = false }) {
  ...
  return <article className={`q49-metric${accent ? ' q49-accent' : ''}`}><span>{label}</span><strong>{pct(data?.successRate, count)}</strong><small>{count > 0 ? `${data?.successCount || 0}/${count} 期未在7码中出现` : '暂无已开奖样本'}</small><em>特别码未出现 {specialPct(data)}{count > 0 ? ` · ${data.specialCodeMissCount ?? '--'}/${count}` : ''}</em></article>;
}
