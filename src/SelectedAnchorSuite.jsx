...
function Stat({ label, data, accent = false }) {
  return (
    <div className={`sas-stat${accent ? " is-accent" : ""}`}>
      ...
      <small>
        {data?.count ? `${data.successCount}/${data.count} 成功` : "等待开奖"}
      </small>
    </div>
  );
}
