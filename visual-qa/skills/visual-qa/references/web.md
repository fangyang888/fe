...
- 先读摘要的 `regionIteration`，优先检查恶化和新增区域；统计是最多 12 个固定截图区域，不等于组件数或整页问题总数。已解决只代表该区域按当前像素阈值已无差异，不能替代 final。
- `domCandidates` 是几何候选。按需读取 report 中对应区域的候选 bounds、styles 和 parent；结合契约实测值再定位原因，不把候选 selector 当成已证实根因。不输出完整 DOM、全部样式或整份 report。
- `diagnosticStrategy=parent-layout-and-fonts` 时，先核对父容器尺寸/间距、字体族/字号/行高和已有契约，再批量修正。下一轮会自动执行 agent 完整检查，不要继续无修改重跑。
- 局部裁片可能已包括附近父容器，先读现有裁片。DOM 检查仅覆盖主文档、最多 5000 个元素；出现 warning 或没有候选时，依据裁片继续诊断，不捏造元素对应关系。
- 用户或设计明确要求关键按钮、文字、图标独立验收时，可在 Web case 配置 `criticalRegions`，使用真实设计 PNG 的整数物理像素坐标及固定阈值。不要根据当前实现反推预期或为过关放宽阈值。未声明时不自动替用户划定关键区域。
